...
export default function ClinicLeadForm() {
  ...
  const canContinue = (): boolean => {
    ...
    if (step === 4) {
      return Boolean(form.name.trim()) && isValidContact(form.contact) && form.consent;
    }
    return false;
  };
  ...
  const submit = async () => {
    if (!canContinue()) {
      if (step === 4) {
        ...
        else if (!isValidContact(form.contact)) setError("ایمیل یا شماره موبایل را درست وارد کنید.");
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          source: "clinic_lead_form",
          page: "/clinic",
          name: form.name.trim(),
          contact: form.contact.trim(),
          goal: form.goal,
          channel: form.goal === "chatbot" ? form.channel : form.service,
          budget: form.budget,
          intent: form.goal,
      ...
      pushGtmEvent("lead_submit", { source: "clinic_lead_form", page: "clinic", goal: form.goal });
      setDone(true);
    } catch {
      ...
      <div className="container-mx container-px">
        <div className="grid items-start gap-10 lg:grid-cols-[1fr_1.05fr] lg:gap-14">
          <div className="text-right">
            <span className="badge mb-4 bg-white/10 text-brand-300">شروع کنیم</span>
            <h2 className="section-title text-white">۳۰ ثانیه تا پیشنهاد اختصاصی کلینیک شما</h2>
            <p className="mt-3 max-w-md text-[15px] leading-relaxed text-navy-200">
              چند سؤال کوتاه می‌پرسیم تا نیاز کلینیک‌تان را بفهمیم. بدون تعهد، بدون تماس مزاحم.
            </p>
            ...
            {done ? (
              <div className="py-6 text-center" role="status" aria-live="polite">
                ...
                <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-navy-500">
                  پیشنهاد اختصاصی کلینیک شما در حال آماده‌سازی است. کارشناسان آرایه طی کمتر از یک
                  روز کاری با شما تماس می‌گیرند.
                </p>
                <ChatOpenButton
                  location="clinic_lead_success"
                  className="mt-5 inline-flex rounded-xl border border-navy-200 px-5 py-2.5 text-sm font-bold text-navy-700 transition hover:border-brand-200 hover:text-brand-600"
                >
              ...
              <>
                ...
                {step === 1 ? (
                  <fieldset>
                    <legend className="mb-3 text-sm font-extrabold text-navy-900">
                      مهم‌ترین نیاز کلینیک شما چیست؟
                    </legend>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {clinicGoalOptions.map((opt) => (
                        <OptionButton
                          key={opt.value}
                  ...
                  <fieldset>
                    <legend className="mb-3 text-sm font-extrabold text-navy-900">
                      {form.goal === "chatbot"
                        ...
                        : "بیشتر روی چه خدماتی تمرکز دارید؟"}
                    </legend>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {(form.goal === "chatbot" ? clinicChannelOptions : clinicServiceOptions).map(
                        (opt) => (
                          <OptionButton
                            key={opt.value}
                            selected={
                ...
                {step === 4 ? (
                  <fieldset className="space-y-4">
                    ...
                    <div>
                      <label htmlFor="clinic-lead-name" className="mb-1.5 block text-xs font-bold text-navy-600">
                        نام شما
                      </label>
                      <input
                        id="clinic-lead-name"
                        type="text"
                        autoComplete="name"
                        placeholder="مثلاً نازنین فرهادی"
                        value={form.name}
                        onChange={(e) => setField("name", e.target.value)}
                        className="w-full rounded-xl border border-navy-100 bg-navy-50/40 px-4 py-2.5 text-sm text-navy-900 outline-none focus:border-brand-300 focus:ring-2 focus:ring-brand-100"
                      />
                    </div>
                    <div>
                      <label htmlFor="clinic-lead-contact" className="mb-1.5 block text-xs font-bold text-navy-600">
                        ایمیل یا شماره موبایل
                      </label>
                      <input
                        id="clinic-lead-contact"
                        type="text"
                        inputMode="email"
                        autoComplete="email"
    ...
    <button
      ...
      className={`rounded-xl border px-4 py-3 text-right text-sm font-bold transition ${
        selected
          ...
          : "border-navy-100 bg-white text-navy-700 hover:border-brand-200 hover:bg-brand-50/40"
      }`}
    >
      {label}
    </button>
