...
export function generateStaticParams() {
  return getPublishedToolPages("bizcard").map((p) => ({ slug: p.slug }));
}

export function generateMetadata({ params }: Props): Metadata {
  const page = buildToolProgrammaticPage("bizcard", params.slug);
  if (!page) return {};
  return toolProgrammaticMetadata(page);
}

export default function BizcardSlugPage({ params }: Props) {
  const page = buildToolProgrammaticPage("bizcard", params.slug);
  if (!page) notFound();
