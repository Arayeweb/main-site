...
export function generateStaticParams() {
  return getPublishedToolPages("seo-roi-calculator").map((page) => ({ slug: page.slug }));
}
export function generateMetadata({ params }: Props): Metadata {
  const page = buildToolProgrammaticPage("seo-roi-calculator", params.slug);
  return page ? toolProgrammaticMetadata(page) : {};
}
export default function SeoRoiIndustryPage({ params }: Props) {
  const page = buildToolProgrammaticPage("seo-roi-calculator", params.slug);
  if (!page) notFound();
  return (
    <>
