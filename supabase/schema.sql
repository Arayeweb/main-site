...
create table if not exists public.ai_users (
  ...
  credits          int  not null default 10,    -- اعتبار باقی‌مانده
...
insert into public.ai_plans (id, name, price_toman, credits, description, features, is_active, is_featured) values
  ('free', 'رایگان', 0, 10, 'رایگان امتحان کنید — چند گفت‌وگوی اولیه برای آشنایی با آرایه AI', '["رایگان امتحان کنید", "چند گفت‌وگوی اولیه", "فقط مدل‌های اقتصادی"]', true, false),
  ('starter', 'شروع', 99000, 80, 'مناسب تست و استفاده سبک', '["۸۰ کردیت", "مدل‌های اقتصادی"]', true, false),
  ('plus', 'پلاس', 299000, 260, 'مناسب استفاده روزمره', '["۲۶۰ کردیت", "همه مدل‌های چت"]', true, false),
  ('pro', 'حرفه‌ای', 649000, 600, 'بهترین گزینه برای اکثر کاربران', '["۶۰۰ کردیت", "محبوب‌ترین"]', true, true),
  ('max', 'مکس', 1290000, 1250, 'مناسب مصرف بالا', '["۱۲۵۰ کردیت", "مصرف بالا"]', true, false),
  ('team_mini', 'تیم کوچک', 1900000, 2500, 'تیم‌های کوچک', '["۲۵۰۰ کردیت", "۳ کاربر"]', false, false),
  ('business', 'سازمانی', 4900000, 6500, 'تیم و مصرف بالا', '["۶۵۰۰+ کردیت", "اتصال اختصاصی"]', false, false)
on conflict (id) do nothing;
...
create table if not exists public.ai_runs (
  ...
  total_provider_cost_usd numeric(12,8),
  total_revenue_toman int,
  total_gross_profit_toman int,
...
create table if not exists public.model_calls (
  ...
  cached_tokens   int,
  tool_cost_usd   numeric(12,8),
  cost_usd        numeric(12,8),
  exchange_rate_toman int,
  pricing_multiplier numeric(8,4),
  credit_value_toman int,
  credits_charged int not null default 0,
  revenue_toman   int,
  gross_profit_toman int,
...
-- ---------- Cost-based credits / expiring lots overrides (20260726) ----------
...
begin
  ...
  loop
    ...
    update public.ai_credit_lots set remaining = remaining - v_take where id = v_lot.id;
...
begin
  ...
  insert into public.ai_runs (id, user_id, conversation_id, mode, status, reserved_credits, metadata)
  values (p_run_id, p_user_id, v_conv, p_mode, 'running', p_reserved_credits, coalesce(p_metadata, '{}'::jsonb));
